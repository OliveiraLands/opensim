...
* `aas export-legacy <path> [--overwrite]`: Exporta todos os assets do serviço para uma estrutura de pastas compatível com `FSAssetService` (por padrão, apenas exporta os arquivos não existentes).
...
* `aas restore-from-log <log_path> <fs_path>`: Escaneia um arquivo de log (ex: Robust.log) buscando avisos de assets não encontrados e tenta importá-los automaticamente a partir da pasta FSAsset legada.
* `aas generate-dummies [--dry-run]`: Escaneia todos os itens de inventário e gera assets falsos (dummy placeholders) com o formato correto para cada tipo de asset que esteja permanentemente perdido.
